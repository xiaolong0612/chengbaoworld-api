    ...
    public function handle(Request $request, \Closure $next, $force = true)
    {
        ...
        try {
            ...
            if (!$token || strlen($token) != 40) {
                throw new ApiException('token错误1', StatusCode::LOGIN_CODE);
            }
            $token = $usersRepository->getToken($token);
            if (!$token) {
                throw new ApiException('token错误2', StatusCode::LOGIN_CODE);
            }

            $service = new JwtService();
            $payload = $service->decode($token);
            if (!$payload) {
                throw new ApiException('token错误3', StatusCode::LOGIN_CODE);
            }
            $userId = $payload['data']->uid;
            $isLogin = Cache::store('redis')->get('token_'.$userId);
            if($isLogin != $request->header('token')){
                throw new ApiException('token错误4', StatusCode::LOGIN_CODE);
            }
