...
class Active extends Base
{
    ...
    public function commons(){
        ...
        $synList = $activeSynRepository->search([],$this->request->companyId)->select();
        $poolList = $this->app->make(PoolSaleRepository::class)->search(['is_number'=>1],$this->request->companyId)->field('id,title')->select();
        $this->assign(['synList'=>$synList,'poolList'=>$poolList]);
    }
    ...
    public function list()
    {
        ...
        return $this->fetch('/active/active/list', [
            ...
            'switchIsDefaultAuth' => company_auth('companyActiveDefaultSwitch'),
        ]);
    }
    ...
    public function default()
    {
        ...
        if ($this->request->isPost()) {
            ...
            try {
                $res = $this->repository->update($id, $param);
                company_user_log(3, '黙认活动状态 id:' . $id, $param);
                if ($res !== false) {
                    return $this->success('修改成功');
                } else {
                    return $this->error('修改失败');
