 ...
 * 无限极联动选择组件封装模块
 */
layui.define(['jquery', 'form', 'multiSelect'], function (exports) {
    var $ = layui.$, form = layui.form, multiSelect = layui.multiSelect;
    // 监听选中事件
    ...
    var CascadeSelect = {
        init: function (initObj, isReset) {
            ...
            $.get(url, function (res) {
                ...
                selectValue != -1 && $("select[name='" + initObj + "']").val(selectValue);
            ...
            if (id !== "") {
                $.ajax({
                    ...
                    success: function (res) {
                        ...
                        if (data.length > 0) {
                            $.each(data, function (i, d) {
                                _this.append("<option value='" + d.id + "'>" + d.name + "</option>");
                            });
                            if (!isReset) {
